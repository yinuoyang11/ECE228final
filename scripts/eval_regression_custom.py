...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Override checkpoint LIBERO episode list. Defaults to whatever was used during training.",
    )
    ...
    episodes = args.episodes if args.episodes is not None else train_args.get("episodes")
    skip_samples = int(args.skip_samples if args.skip_samples is not None else train_args.get("max_samples", 0))

    local_dir = args.local_dir or train_args.get("local_dir")
